            ...
            for role_for_app in plex_roles:
                for role_in_guild in roles_in_guild:
                    ...
                    if role is not None and (role in after.roles and role not in before.roles):
                        # Check if user is already in DB (restricted)
                        existing_email = db.get_useremail(str(after.id))
                        
                        if existing_email:
                            print(f"User {after.name} regained role, unrestricting Plex access.")
                            if plexhelper.plex_unrestrict_user(plex, existing_email):
                                await embedinfo(after, 'Your Plex access has been restored!')
                            else:
                                await embederror(after, 'There was an error restoring your Plex access. Please contact an admin.')
                        else:
                        ...
                        try:
                            ...
                            if email:
                                if plexhelper.plex_restrict_user(plex, email):
                                    ...
                                    print("Cannot restrict Plex for this user.")
                                    await embederror(after, "Could not restrict Plex access. Please contact admin.")
                            else:
                                print("No email found for user {} to restrict.".format(after.name))
                        except Exception as e:
                            print(e)
                            print("{} Cannot restrict this user in plex.".format(email if 'email' in locals() else 'Unknown'))
                        plex_processed = True
                        break
                if plex_processed:
                    break
